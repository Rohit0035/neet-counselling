...
import BlogCategoryForm from "./NoticeCategoryForm";
import axios from "axios";
import toast from "react-hot-toast";
import Swal from "sweetalert2";
import { useLoader } from "@/context/LoaderContext";

export default function NoticeCategoryTable({
  show,
  setShow,
  editData,
  setEditData,
  ...
  const toggleStatus = async (id, status) => {
    ...
    try {
      setLoading(true);
      await axios.put(`/api/notice-categories/${id}`, {
        status: !status,
      });
  ...
  const handleDelete = async (item) => {
    ...
    try {
      setLoading(true);
      await axios.delete(
        `/api/notice-categories/${item._id}`
      );
